    ...
    def get(self, img, max_num=0, det_metric='default'):
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                # First pass: generate keypoints if we have landmark models
                if face.kps is None:
                    for taskname, model in self.models.items():
                        if taskname in ['landmark_2d_106', 'landmark_3d_68']:
                            try:
                                print(f"Generating keypoints with {taskname} model...")
                                model.get(img, face)
                                if hasattr(face, 'kps') and face.kps is not None:
                                    print(f"✅ Keypoints generated by {taskname}: shape {face.kps.shape}")
                                    break
                            except Exception as e:
                                print(f"⚠️ Error generating keypoints with {taskname}: {e}")
                
                # If still no keypoints, generate approximate ones from bbox
                if face.kps is None:
                    print("Generating approximate keypoints from bounding box...")
                    face.kps = self.generate_approximate_keypoints(face.bbox)
                    print(f"✅ Generated approximate keypoints: shape {face.kps.shape}")
                
                # Second pass: process other models (recognition, gender/age)
                for taskname, model in self.models.items():
                    if taskname == 'detection' or model == 'yolo' or taskname in ['landmark_2d_106', 'landmark_3d_68']:
                        continue
                    try:
                        print(f"Processing face with {taskname} model...")
                        model.get(img, face)
                        # Check if this model generated an embedding
                        if hasattr(face, 'embedding') and face.embedding is not None:
                            embedding_generated = True
                            print(f"✅ Embedding generated by {taskname} model: shape {face.embedding.shape}")
                        else:
                            print(f"⚠️ No embedding from {taskname} model")
                    except Exception as e:
                ...
                if not embedding_generated:
                    print(f"⚠️ No embedding generated for face, setting to None")
                    face.embedding = None
                else:
                    print(f"✅ Face processed successfully with embedding")
