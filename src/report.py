...
def get_attendance_data(start_date=None, end_date=None, student_name=None, limit=1000):
    ...
    query_parts = ["SELECT name, timestamp, confidence, device_id FROM attendance_records"]
    params = []
...
def get_class_attendance_data(start_date=None, end_date=None, student_name=None, subject=None, teacher_subjects=None, section=None):
    ...
    if start_date:
        where_clauses.append("ca.class_date >= ?")
        params.append(start_date)
    ...
    if student_name and student_name != "All Students":
        where_clauses.append("ca.student_name = ?")
        params.append(student_name)
        
    if subject and subject != "All Subjects":
        where_clauses.append("ca.subject = ?")
    ...
    if teacher_subjects and "All Subjects" not in teacher_subjects:
        ...
        where_clauses.append(f"ca.subject IN ({placeholders})")
...
def get_registered_students():
    conn = get_db_connection()
    query = "SELECT name FROM student_profiles ORDER BY name"
...
def add_manual_attendance(student_name, class_date, subject, start_time, end_time, attended, class_type='lec'):
    ...
    try:
        ...
        if not class_exists:
            cursor.execute("""
                INSERT INTO class_schedules (day, subject, start_time, end_time, type)
                VALUES (?, ?, ?, ?, ?)
        ...
        if attended:
            ...
            try:
                ...
                # Add new log entry
                cursor.execute("""
                    INSERT INTO attendance_records (name, timestamp, confidence, device_id, day_of_week)
                    VALUES (?, ?, ?, ?, ?)
                """, (
                    student_name, 
                    timestamp, 
...
def get_attendance_summary(start_date=None, end_date=None, search_term=None, sort_by="student_name", sort_dir="asc", limit=100, offset=0, teacher_subjects=None, section=None):
    """Get attendance summary with pagination, search, section and sorting options"""
    conn = get_db_connection()
    ...
    section_condition = ""
    if section and section != "All Sections":
        section_condition = "AND s.section = ?"
        params.append(section)
...
def get_attendance_outliers(start_date=None, end_date=None, limit=5, teacher_subjects=None, section=None):
...
def get_sections():
    ...
    conn = get_db_connection()
    # Update the table name FROM student_profiles to student_profiles
    query = "SELECT DISTINCT section FROM student_profiles WHERE section IS NOT NULL ORDER BY section"
    df = pd.read_sql(query, conn)
    conn.close()
    return ["All Sections"] + df['section'].tolist()
